...
probe_eff <- function(df, params, orient = "u", method = "blunt pipe") {
 # cat("This function replaces the eff_probe column if it already exists")
 #  cat("\n")
  stopifnot("Only blunt pipe method currently available" = method == "blunt pipe")

  R <- (df$D_p * 1e-4)^2 * 9.807/(18 * params$viscosity_air * params$velocity_air)
  # The stokes number in Su is different from our base Stk by 1/2
  p <- 2.2 * R^(1.3) * df$Stk / 2
  q <- 75 * R^1.7 * df$Stk / 2
  # ******** alpha, beta and B are from Ref 6: Thin-walled probe facing downwards: alpha =
  # 1; beta = 0; B = 1 Thin-walled probe facing upwards: alpha = 0; beta = 1; B = 1
  # Horizontal thin-walled probe: alpha = 0.8; beta = 0.2; B = 1

alpha <- dplyr::case_when(orient == "u" ~ 0,
                          orient == "d" ~ 1,
                          orient == "h" ~ 0.8)
beta <- dplyr::case_when(orient == "u" ~ 1,
                          orient == "d" ~ 0,
                          orient == "h" ~ 0.2)
p <- 2.2 * R^(1.3) * df$Stk / 2
q <- 75 * R^1.7 * df$Stk / 2
  B <- 1  #for all thin-walled probes
  df$eff_probe <- (1 - 0.8 * (4 * df$Stk / 2 * R^(3/2)) +
              0.08 * (4 * df$Stk / 2 * R^(3/2))^2 - alpha *
              ((0.5 * R^(1/2)) - R * (B^2 - 1)) -
              beta * (0.12 * R^-0.4 * (exp(-p) - exp(-q)) - R^(3/2) *
              (B^(1/2) - 1)))
# all the bonkers results set to zero
  ...
  # ggplot(df, aes(D_p, eff_probe)) + geom_line()
  # to compare to reference results in Su and Vincent:
