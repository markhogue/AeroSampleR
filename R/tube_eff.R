...
tube_eff <- function(df, params, L, angle_to_horiz, elnum) {
 # cat("This function appends a new eff_tube.. column to the data frame")
 #  cat("\n")

  angle_to_horiz_radians <- angle_to_horiz * pi/180
  # Depo_Calc Eq 46:
  Dc <- params$k * params$T_K * df$C_c /
        (3 * pi * params$viscosity_air * 1e-6 * df$D_p)
  ...
  # Depo_Calc Eq 31:
  t_prime <- L * df$v_ts / (params$velocity_air * params$D_tube) *
     cos(angle_to_horiz_radians)

  # Depo_Calc Eq 39:
  t_plus <- 0.0395 * df$Stk * params$Re^(3/4)

  # Depo_Calc Eq 42:
  V_plus <- 0.0006 * t_plus^2 + 2e-8 * params$Re
  ...
  if(params$Re >= 2100 && params$Re < 4000 &&
